...
export function NewTransactionModal({ isOpen, onClose, initialAccountId, initialData, isCloning }: NewTransactionModalProps) {
    ...
    // Auto-seleciona a conta recém-criada quando é a única disponível
    useEffect(() => {
        const currentTotal = bankAccounts.length + creditCards.length;
        const prevTotal = prevAccountsTotalRef.current;

        if (currentTotal > prevTotal && !accountId) {
            const allAccounts = type === 'INCOME'
                ? bankAccounts
                            ...
                            <>
                                ...
                                <div className="flex-1 overflow-y-auto p-8 bg-white">
                                    <div className="flex flex-col gap-6">
                                        ...
                                        {/* 6. Parcelas (Despesa + Cartão de Crédito) */}
