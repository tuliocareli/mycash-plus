...
export function UpcomingExpensesWidget() {
    ...
    const handleMarkAsPaid = async (transaction: Transaction) => {
        ...
        setTimeout(async () => {
            ...
            // 2. Recurrence Logic
            // If it's recurring (single transaction repeated monthly conceptually? Or distinct rows generated?)
            // The schema has `isRecurring`. If true, do we auto-generate next?
            // Usually recurring transactions are generated by a backend job or when viewing future months.
            // For now, let's keep the existing logic of creating the next occurrence manually if that was the intend.
            // But "isRecurring" usually implies a definition exists.

            // If we assume manual duplication for next month:
            ...
            if (transaction.totalInstallments > 1 && transaction.installmentNumber) {
    ...
    const getOriginLabel = (accountId?: string) => {
        if (!accountId) return 'Origem não identificada';
                            ...
                            >
                                <div className="flex flex-col gap-1">
                                    <span className="text-base font-bold text-neutral-1100">
                                        {expense.description}
                                        {expense.installmentNumber && expense.totalInstallments > 1 && (
