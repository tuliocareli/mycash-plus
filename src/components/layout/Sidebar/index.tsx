...
export function Sidebar({ isCollapsed, toggleSidebar }: SidebarProps) {
    ...
    return (
      ...
      */}
            <div className={clsx(
                'flex items-center mb-12 shrink-0 transition-all duration-300 min-h-[40px] overflow-hidden',
                isCollapsed ? 'justify-center w-full px-4' : 'w-full gap-3'
            )}>
                 {/* Novo Logo Oficial: SVG Completo (Ícone + Texto) */}
                 <div className={clsx(
                     "relative shrink-0 transition-all duration-300",
                     isCollapsed ? "w-12 h-12 overflow-hidden" : "w-full"
                 )}>
                    <svg className={clsx(
                        "h-12 w-auto",
                        isCollapsed ? "min-w-[160px] translate-x-[-12px]" : ""
                    )} viewBox="0 0 832.1 249.8" fill="none" xmlns="http://www.w3.org/2000/svg">
