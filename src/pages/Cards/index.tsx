    ...
    return (
        <div className="w-full flex flex-col gap-8 pb-20 animate-fade-in">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-neutral-1100">Contas e Cartões</h1>
                    <p className="text-neutral-500 mt-1">Gerencie seus saldos, limites e faturas.</p>
                </div>
                <button
                    onClick={() => { setSelectedCard(null); setIsAddCardOpen(true); }}
            ...
            <div className="flex flex-col gap-6">
                <div className="flex items-center gap-2">
                    <div className="size-8 rounded-lg bg-neutral-100 flex items-center justify-center text-neutral-600">
                        <CardIcon size={18} />
                    </div>
                    <h2 className="text-xl font-bold text-neutral-1100">Contas Bancárias</h2>
                </div>

                {bankAccounts.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-8 bg-neutral-100 rounded-3xl border border-dashed border-neutral-300 text-center gap-2">
                        <p className="text-neutral-500 text-sm">Nenhuma conta bancária cadastrada.</p>
                        <button
                            onClick={() => { setSelectedCard(null); setIsAddCardOpen(true); }}
                            className="text-brand-700 font-bold hover:underline text-sm"
                        >
                            Adicionar Conta
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {bankAccounts.map(acc => (
                            <div
                                ...
                                className="group bg-white rounded-3xl border border-neutral-200 p-6 flex items-center gap-4 cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all duration-300 relative overflow-hidden"
                            >
                                <div className={clsx(
                                    "size-12 rounded-2xl flex items-center justify-center shadow-sm border",
                                    acc.theme === 'black' ? "bg-neutral-1100 text-white border-transparent" :
                                        acc.theme === 'lime' ? "bg-brand-500 text-neutral-1100 border-transparent" :
                                            "bg-white text-neutral-1100 border-neutral-200"
                                )}>
                                    {acc.logoUrl ? (
                                        <img src={acc.logoUrl} alt={acc.bank} className="size-6 object-contain" />
                                    ) : (
                                        <CardIcon size={24} />
                                    )}
                                </div>
                                <div className="flex flex-col flex-1 min-w-0">
                                    <span className="text-xs font-bold text-neutral-400 uppercase truncate">{acc.bank}</span>
                                    <h3 className="text-base font-bold text-neutral-1100 truncate">{acc.name}</h3>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs font-bold text-neutral-400">Saldo</p>
                                    <p className="text-lg font-extrabold text-neutral-1100">{formatCurrency(acc.balance)}</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="flex flex-col gap-6">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-bold text-neutral-1100">Cartões de Crédito</h2>
                </div>

                {sortedCards.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-8 bg-neutral-100 rounded-3xl border border-dashed border-neutral-300 text-center gap-2">
                        <p className="text-neutral-500 text-sm">Nenhum cartão de crédito cadastrado.</p>
                        <button
                            onClick={() => { setSelectedCard(null); setIsAddCardOpen(true); }}
                            className="text-brand-700 font-bold hover:underline text-sm"
                        >
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-start pt-2">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-bold text-neutral-400 uppercase tracking-wider mb-1">{card.bank}</span>
                                            <h3 className="text-xl font-bold text-neutral-1100 leading-tight">{card.name}</h3>
                                        </div>
                                        <div className={clsx(
                                            "size-10 rounded-xl flex items-center justify-center shadow-sm border overflow-hidden",
                                            ...
                                            {card.logoUrl ? (
                                                <img src={card.logoUrl} alt={card.bank} className="size-6 object-contain" />
                                            ) : (
                                                <CardIcon size={20} />
                                            )}
                                        </div>
